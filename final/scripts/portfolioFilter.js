...
function renderProjects(projectsList, container) {
    ...
    <article class="service-card project-card">
      ...
      <div style="margin-top: 1rem; display: flex; gap: 1rem;">
        <a href="${project.demoUrl}" class="card-link">Live View &rarr;</a>
        <a href="${project.githubUrl}" target="_blank" rel="noopener" class="card-link" style="color: var(--text-muted);">Source &rarr;</a>
      </div>
    </article>
  `).join('');
}